...
ExportSeriesImageWidget_PLC::ExportSeriesImageWidget_PLC(QWidget *parent)
	: QWidget(parent)
{
	setupUI();
	connectSlots();
...
ExportSeriesImageWidget_PLC::~ExportSeriesImageWidget_PLC()
{
...
void ExportSeriesImageWidget_PLC::connectSlots()
{
	bool is_ok = false;

	is_ok = connect(spinbox_a_total_number_, QOverload<int>::of(&QSpinBox::valueChanged),
					this, &ExportSeriesImageWidget_PLC::SlotATotalValueChanged);

	is_ok = connect(spinbox_x0_total_number_, QOverload<int>::of(&QSpinBox::valueChanged),
					this, &ExportSeriesImageWidget_PLC::SlotX0TotalValueChanged);

	is_ok = connect(spinbox_c_total_number_, QOverload<int>::of(&QSpinBox::valueChanged),
					this, &ExportSeriesImageWidget_PLC::SlotCTotalValueChanged);

	is_ok = connect(pushbutton_save_series_images_, &QPushButton::clicked,
					this, &ExportSeriesImageWidget_PLC::SlotSaveSeriesImages);
}



void ExportSeriesImageWidget_PLC::setupUI()
{
	...
	doublespinbox_a_start_->setSingleStep(0);
...
void ExportSeriesImageWidget_PLC::SlotSaveSeriesImages()
{
	// 导出的文件名
	...
	for (int i = 0; i < image_total_number; ++i)
	{
		...
		// 采样点在参考图中的位置
		const rowvec refer_x = InverseMap::InverseMap_PLCDeformation(a, x0, c, width);
...
void ExportSeriesImageWidget_PLC::SlotATotalValueChanged(int total_number)
{
...
void ExportSeriesImageWidget_PLC::SlotX0TotalValueChanged(int total_number)
{
...
void ExportSeriesImageWidget_PLC::SlotCTotalValueChanged(int total_number)
{
	spinbox_a_total_number_->setValue(total_number);
	spinbox_x0_total_number_->setValue(total_number);
}
